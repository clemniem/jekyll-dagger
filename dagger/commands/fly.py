...
class Fly(Base):
	...
	def run(self):
		...
		if os.path.isfile(target) and target.lower().endswith(('.md','.markdown')):
			...
			targets = [f for f in os.listdir(target) if os.path.isfile(os.path.join(target, f)) and f.lower().endswith(('.md','.markdown'))]

		if not targets:
			print("No scribbles found in {}, please check for valid scribbles.".format(target))
		else:
			#fly to all targets and dagger them
			for f in targets:
				self.fly(f)